...
class Menu < Gosu::Window
    attr_accessor :screen, :records, :playing_count

    def initialize(screen, records, playing_count)
        super(1200, 800)
        ...
        @image_bloodmoon_logo = Gosu::Image.new('Media/Image_Bloodmoon_Logo.png')
        ...
        @screen_1 = 0                                                             # Set up the variable for hiding/showing speech bubble
        ...
        if (@screen == 1)                                                        # When in the main screen
            if (id == Gosu::MsRight)                                              # Check if player right-click to the each game's button
                ...
                if (game1(mouse_x, mouse_y)) && (@screen_1 == 0)
                    close
                    GemsCollection.new(@records, @playing_count).show
                elsif (game2(mouse_x, mouse_y)) && (@screen_1 == 0)
                    close
                    StarWars.new(@records, @playing_count).show
                elsif (game3(mouse_x, mouse_y)) && (@screen_1 == 0)
                    close
                    Original2048.new(@records, @playing_count).show
                elsif (game4(mouse_x, mouse_y)) && (@screen_1 == 0)
                    close
                    OvercomingObstacles.new(@records, @playing_count).show
                elsif (Gosu.distance(mouse_x, mouse_y, 50, 750) <= 30)            # Check if player right-click to the information button
                    if (@screen_1 == 0)                                
                ...
                elsif (Gosu.distance(mouse_x, mouse_y, 1150, 750) <= 30) && (@screen_1 == 0)
                    @screen = 2
                end
            end
    ...
    def draw
        ...
        if (@screen == 0)
            ...
            @image_bloodmoon_logo.draw(@x_2, @y_2, @z_2, 0.2, 0.2)
        ...
        # Main screen drawing
            ...
            # Draw all four games' buttons and change the mouse pointer
            if game1(mouse_x, mouse_y) && (@screen_1 == 0)
            ...
            elsif ((Gosu.distance(mouse_x, mouse_y, 50, 750) <= 30) || (Gosu.distance(mouse_x, mouse_y, 1150, 750) <= 30)) && (@screen_1 == 0)
                @image_mp_3.draw(mouse_x, mouse_y, 1)
            elsif (@screen_1 == 0)
            ...
            @font.draw('<i>(By BLOODMOON)</i>', 500, 145, 0, 0.9, 0.9, Gosu::Color::CYAN)
            @font.draw('Gems Collection', 350, 420, 0, 0.8, 0.8, Gosu::Color::YELLOW) if (!game1(mouse_x, mouse_y)) || (@screen_1 == 1)
            ...
            if (@screen_1 == 1)
                ...
                if (@info_y > 20)
                    ...
                    @font.draw('<i>My name is Trung Kien Nguyen - BLOODMOON - Student ID 104053642 - Author of this program!!</i>', 180, 86, 0, 0.6, 0.6, Gosu::Color::BLACK)
                    ...
                    @font.draw('<i>This is the custom program for COS10009 Introduction to Programming - GAMES MENU</i>', 180, 158, 0, 0.6, 0.6, Gosu::Color::BLACK)
            ...
            # Draw the back-to-home icon and change the mouse pointer
...
Menu.new(0, records, 0).show                                      # Open the Main games menu window with the intro screen appearance first